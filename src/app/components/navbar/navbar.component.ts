  ...
  template: `
    <nav class="fixed top-0 w-full z-50 glass-panel border-b-0 rounded-none px-6 py-4 flex justify-between items-center transition-all duration-300">
      <div class="flex items-center gap-2 cursor-pointer" routerLink="/">
        ...
        <span class="font-mono font-bold text-xl tracking-wider text-gray-900 dark:text-white transition-colors duration-300">SYS.ARCHITECT</span>
      </div>
      ...
      <div class="hidden md:flex gap-8 items-center">
        ...
        <!-- Theme Toggle -->
        <button mat-icon-button (click)="toggleTheme()" class="text-gray-700 dark:text-gray-300 ml-4 transition-colors duration-300">
      ...
      <div class="flex items-center md:hidden gap-2">
        <!-- Theme Toggle Mobile -->
        <button mat-icon-button (click)="toggleTheme()" class="text-gray-700 dark:text-gray-300 transition-colors duration-300">
          <mat-icon>{{ isDarkMode() ? 'light_mode' : 'dark_mode' }}</mat-icon>
        </button>
        
        <!-- Menu Toggle -->
        <button mat-icon-button class="!text-cyber-cyan" (click)="toggleMenu()">
